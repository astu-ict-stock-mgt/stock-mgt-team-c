...
export async function listAuditLogs(params: { page: number; limit: number; search?: string; userId?: string; module?: string; action?: string; startDate?: string; endDate?: string }) {
  ...
  if (params.search) {
    where.OR = [
      { action: { contains: params.search } }, { module: { contains: params.search } },
      { description: { contains: params.search } }, { entity: { contains: params.search } },
    ];
  }
  ...
  return { total, items: items.map((a) => ({
