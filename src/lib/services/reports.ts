...
export async function inventoryReport(params: {
  ...
  for (const it of items) {
    ...
    out.push({
      ...
      unitCost: val.avgUnitCost,
      totalValue: val.value,
      minStock: it.minStock,
      maxStock: it.maxStock,
      reorderLevel: it.reorderLevel,
      safetyStock: it.safetyStock,
      isLowStock: val.quantity <= it.reorderLevel,
      isOutOfStock: val.quantity <= 0,
    });
  }
  return {
...
export async function auditReport(params: {
  startDate?: string;
  endDate?: string;
  userId?: string;
  module?: string;
