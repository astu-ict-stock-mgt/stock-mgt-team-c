...
export async function getRole(id: string) {
  ...
  return {
    ...
    permissionIds: role.permissions.map((rp) => ({ roleId: rp.roleId, permissionId: rp.permissionId, name: rp.permission.name })),
  };
}
...
export async function createRole(input: {
  ...
  const existing = await db.role.findUnique({ where: { name: input.name.toUpperCase() } });
  ...
  const role = await db.$transaction(async (tx) => {
    ...
    if (input.permissionIds && input.permissionIds.length) {
      await tx.rolePermission.createMany({
        data: input.permissionIds.map((pid) => ({ roleId: r.id, permissionId: pid })),
      });
    }
    return r;
  });
  ...
  await recordAudit({
    ...
    entity: "role",
    entityId: role.id,
    newValue: { name: role.name, description: role.description, permissionCount: input.permissionIds?.length ?? 0 },
  });
...
export async function updateRole(id: string, input: {
  ...
  if (input.name) {
    const conflict = await db.role.findFirst({ where: { name: input.name.toUpperCase(), NOT: { id } } });
...
export async function togglePermission(
  ...
  // Prevent removing critical permissions from Administrator
  if (role.name === "ADMINISTRATOR" && !enable) {
    throw Errors.forbidden("Cannot remove permissions from the Administrator role");
  }
  ...
  if (enable && existing) return getRole(roleId); // already enabled
  if (!enable && !existing) return getRole(roleId); // already disabled
...
export async function deleteRole(id: string, auditCtx?: { userId?: string }) {
  ...
  if (["ADMINISTRATOR", "PAO", "STOREKEEPER", "STOCK_CLERK", "ACCOUNTANT", "DEPARTMENT_HEAD", "SECURITY_OFFICER", "SUPPLIER"].includes(existing.name)) {
    throw Errors.forbidden("Cannot delete a built-in system role");
  }
  if (existing._count.users > 0) {
    throw Errors.conflict("Cannot delete a role that has users assigned — reassign them first");
  }
...
export async function listAllPermissions() {
  const items = await db.permission.findMany({ orderBy: { module: "asc" } });
